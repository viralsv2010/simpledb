...
public class TupleDesc {
	...
	public Type getType(int i) throws NoSuchElementException,UnsupportedOperationException {
		// returning the type of the Integer i.
		return types[i];
	...
	public String toString() {
		// desc_descriptor is a StringBuffer with which we can modify the String.
		StringBuffer desc_descriptor = new StringBuffer();
		
		// Loop to append the Field type and Field name in one String Buffer.
		int LengthofNamesArray = names.length;
		for(int i= 0; i< LengthofNamesArray; i++)
		{
			desc_descriptor.append(types[i]);
			desc_descriptor.append("(");
			desc_descriptor.append(names[i]);
			desc_descriptor.append(")");
			// If 'i' is the Last Element in the StringBuffer
			if(i == LengthofNamesArray-1)
			{
				break;
			}
			else
		...
		// Converting the StringBuffer to String by toString() Method.
		String describer = desc_descriptor.toString();
		
		System.out.println("Describer String :: " + describer);
			
		return desc_descriptor.toString();
